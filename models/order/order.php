...
class Order
{
    ...
    public function getAllOrders($filters = [], $limit = 50, $offset = 0)
    {
        ...
        $staffNameSelect = orderListStaffNameSelectSql();
        $storeAddressJoin = orderListExoticAddressJoinSql();
        $sql = "SELECT vp_orders.id as order_id, vp_orders.*, purchase_orders.id, purchase_orders.po_number, purchase_orders.vendor_id, purchase_orders.po_date, purchase_orders.expected_delivery_date, purchase_orders.total_cost, vp_vendors.vendor_name as vendor_name, {$staffNameSelect},
    ...
    public function recoverOverriddenOrderStatuses(bool $dryRun = false, int $userId = 0): array
    {
        ...
        // Strategy 1: Recover from vp_order_status_log entries (primary source of truth for local status history)
        $sqlLogs = "SELECT osl.id AS log_id, osl.order_id, osl.status AS log_status, osl.api_response, osl.change_date,
                    ...
                    ORDER BY osl.order_id ASC, osl.id DESC";
        ...
        if ($resLogs && $resLogs !== true) {
            while ($row = $resLogs->fetch_assoc()) {
                ...
                if (isset($candidatesMap[$lineId])) {
                    continue; // Already resolved for this order line
                }

                $logStatus = strtolower(trim((string)($row['log_status'] ?? '')));
                $apiResp = json_decode((string)($row['api_response'] ?? ''), true);

                // Option 1A: Extract explicit old_status from status sync API response JSON
                if (is_array($apiResp) && !empty($apiResp['old_status'])) {
                    $oldStatus = strtolower(trim((string)$apiResp['old_status']));
                    $newStatus = strtolower(trim((string)($apiResp['new_status'] ?? '')));
                    ...
                    if ($oldStatus !== '' && $oldStatus !== 'pending' && ($newStatus === 'pending' || $newStatus === '')) {
                        $candidatesMap[$lineId] = [
                            ...
                            'reason' => 'Status sync override recovery (old_status in vp_order_status_log ID #' . $row['log_id'] . ')',
                        ];
                        continue;
                    }
                }

                // Option 1B: Use the most recent non-pending status logged in vp_order_status_log
                if ($logStatus !== '' && $logStatus !== 'pending') {
                    $candidatesMap[$lineId] = [
                        ...
                        'sku' => (string)($row['sku'] ?? ''),
                        'current_status' => 'pending',
                        'restored_status' => $logStatus,
                        'reason' => 'Historical status recovery (last non-pending log in vp_order_status_log ID #' . $row['log_id'] . ')',
                    ];
                }
            }
        }
