...
class StockTransfer
{
    ...
    public function removeBulkImportOpeningByRef(string $ref_id): bool
    {
        ...
        if ($pid > 0) {
            $this->syncProductPhysicalStockFromLatestMovement($pid);
        }
    ...
    public function upsertBulkImportOpeningStock(
        ...
        if ($oldWh !== $newWh) {
            ...
            $this->syncProductPhysicalStockFromLatestMovement($product_id);
    ...
    public function adjustGrnStockMovement($grnId, $newQty)
    {
        ...
        if (!empty($targetMovement['sku'])) {
            $this->syncProductPhysicalStock($targetMovement['sku'], (int)($grn['product_id'] ?? 0));
        }
    ...
    public function deleteStockTransfer(int $transferId): array
    {
        ...
        foreach ($transfer['items'] ?? [] as $item) {
            ...
            if ($sku !== '' && $pid > 0 && !isset($seenSku[$sku])) {
                $seenSku[$sku] = true;
                $this->syncProductPhysicalStock($sku, $pid);
            }
        }
    ...
    public function syncTransferOutMovements($transferOrderNo, $fromWarehouse, $items, $userId)
    {
        ...
        try {
            ...
            foreach ($existingMovements as $leftover) {
                ...
                $this->syncProductPhysicalStock($leftover['sku'], (int)$leftover['product_id']);
            }
