...
class product
{
            ...
            foreach ($productData as $product) {
                ...
                if (!$variantsAlreadyExpanded && isset($product['variations']) && is_array($product['variations'])) {
                    foreach ($product['variations'] as $variation) {
                        ...
                        $stmt = $this->db->prepare($this->apiRefreshCatalogUpdateSql(false));
                        if ($stmt) {
    ...
    public function getStockLedgerDisplayForMovement(array $row): array
    {
        ...
        if ($mt === 'IN' && $rt === 'ORDER_CANCEL') {
            return [
                'ledger_type' => 'Order cancelled',
                'icon' => 'fa-undo',
                'text_color_class' => 'text-amber-600',
            ];
        }
        if ($mt === 'IN' && $rt === 'ORDER_RETURN') {
            return [
                'ledger_type' => 'Order returned',
                'icon' => 'fa-rotate-left',
