            ...
            addToInvoiceBtn.addEventListener('click', function() {
                ...
                if (!currentBox) {
                    ...
                    newOrderDiv.innerHTML = `
                        ...
                        <div id="availableCourierCompaniess" class="px-4 py-2 text-xs text-gray-700 border-t border-gray-200">
                        </div>
                    `;
        ...
        function fetchCouriersForBox(boxElement) {
            ...
            .then(data => {
                ...
                if (data.success && data.couriers && data.couriers.length > 0) {
                    ...
                    // Separate Delhivery from other couriers (case-insensitive)
                    const delhiveryCouriers = data.couriers.filter(c => {
                        const pk = (c.partner_code || '').toLowerCase().trim();
                        const id = (c.id || '').toLowerCase().trim();
                        return pk === 'delhivery' || id === 'delhivery';
                    });
                    const otherCouriers = data.couriers.filter(c => {
                        const pk = (c.partner_code || '').toLowerCase().trim();
                        const id = (c.id || '').toLowerCase().trim();
                        return pk !== 'delhivery' && id !== 'delhivery';
                    });
                    ...
                    // Render Delhivery section separately (if available)
                    if (delhiveryCouriers.length > 0) {
                        courierHtml += `
                        <div class="courier-rates-panel-delhivery rounded-xl border-2 border-green-400 bg-gradient-to-b from-green-50 to-white shadow-md overflow-hidden text-[13px] mb-3">
                            <div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between px-3 py-2.5 border-b border-green-200 bg-green-100/40">
                                <div class="flex items-center gap-2 min-w-0">
                                    <span class="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-green-600 text-white shadow-sm" aria-hidden="true">
                                        <i class="fas fa-truck text-sm"></i>
                                    </span>
                                    <div class="min-w-0">
                                        <div class="font-semibold text-green-900 leading-tight">🚚 Delhivery Rates (Tested)</div>
                                        <div class="text-[11px] text-green-800/80 truncate">${delhiveryCouriers.length} option${delhiveryCouriers.length !== 1 ? 's' : ''} · select one</div>
                                    </div>
                                    <span class="shrink-0 inline-flex items-center rounded-full bg-green-200 px-2 py-0.5 text-[11px] font-semibold text-green-900">${delhiveryCouriers.length}</span>
                                </div>
                                <p class="text-[11px] text-green-700 sm:text-right">✓ Tested</p>
                            </div>
                        ...
                        delhiveryCouriers.forEach((courier, idx) => {
                            ...
                            const checkedAttr = idx === 0 && otherCouriers.length === 0 ? ' checked' : '';
                            courierHtml += `
                                <label class="relative flex w-[13.5rem] sm:w-56 shrink-0 flex-col rounded-xl border-2 border-green-300 bg-white p-3 pl-9 shadow-sm cursor-pointer transition-all duration-200 hover:shadow-md hover:border-green-400 has-[:checked]:border-green-500 has-[:checked]:bg-green-50/40 has-[:checked]:ring-2 has-[:checked]:ring-green-400 has-[:checked]:ring-offset-1">
                                    <input type="radio" name="${courierGroupName}" value="${escapeHtml(cid)}" class="courier-tile-radio absolute left-2.5 top-3.5 h-4 w-4 shrink-0 border-gray-300 text-green-600 focus:ring-green-500" data-courier-name="${escapeHtml(String(courier.name ?? ''))}" data-partner-code="${escapeHtml(String(courier.partner_code ?? ''))}" data-product-group="${escapeHtml(String(courier.product_group ?? ''))}" data-product-type="${escapeHtml(String(courier.product_type ?? ''))}" data-partner-account-id="${escapeHtml(String(courier.partner_account_id ?? ''))}"${checkedAttr}/>
                                    ${idx === 0 ? '<span class="absolute right-2 top-2 rounded-full px-2 py-0.5 text-[10px] font-bold uppercase tracking-wide text-white shadow-sm pointer-events-none bg-green-600">Best price</span>' : ''}
                                    ...
                                    <div class="mt-3">
                                        ...
                                        <div class="text-lg font-bold tabular-nums text-green-700">${price}</div>
                                    </div>
                                    <div class="mt-3 flex flex-wrap gap-1.5">
                                        ...
                                        <span class="inline-flex items-center gap-1 rounded-full bg-green-100 px-2 py-0.5 text-[11px] font-medium text-green-800 border border-green-200">
                                            <i class="fas fa-check-circle text-green-600 text-[10px]" aria-hidden="true"></i> Tested
                                        </span>
                                    </div>
                                </label>
                            `;
                    ...
                    // Render other couriers (original format)
                    if (otherCouriers.length > 0) {
                        const n = otherCouriers.length;
                        courierHtml += `
                        <div class="courier-rates-panel rounded-xl border border-gray-200 bg-gradient-to-b from-slate-50 to-white shadow-sm overflow-hidden text-[13px]">
                            <div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between px-3 py-2.5 border-b border-gray-100 bg-white/90">
                                <div class="flex items-center gap-2 min-w-0">
                                    ...
                                    <div class="min-w-0">
                                        <div class="font-semibold text-gray-900 leading-tight">Other Courier Options</div>
                        ...
                        otherCouriers.forEach((courier, idx) => {
                            const rating = courier.rating ? (courier.rating + '/5') : 'N/A';
                            const currency = (courier.currency || 'INR').toUpperCase();
                            const priceSymbol = currency === 'INR' ? '₹ ' : (currency + ' ');
                            const price = courier.price != null && courier.price !== ''
                            ...
                            const etdShort = (etd === 'N/A' || etd === '' || etd == null) ? '—' : String(etd);
                            const cid = courier.id != null ? String(courier.id) : '';
                            const checkedAttr = (delhiveryCouriers.length === 0 && idx === 0) ? ' checked' : '';
                            courierHtml += `
                                <label class="relative flex w-[13.5rem] sm:w-56 shrink-0 flex-col rounded-xl border-2 border-gray-200 bg-white p-3 pl-9 shadow-sm cursor-pointer transition-all duration-200 hover:shadow-md ${tm.courierTileHover} ${tm.courierTileChecked}">
                                    ...
                                    ${(delhiveryCouriers.length === 0 && idx === 0) ? '<span class="absolute right-2 top-2 rounded-full px-2 py-0.5 text-[10px] font-bold uppercase tracking-wide text-white shadow-sm pointer-events-none ' + tm.courierTopPick + '">Top pick</span>' : ''}
                                    <p class="pr-14 text-sm font-semibold leading-snug text-gray-900 line-clamp-2">${escapeHtml(courier.name)}</p>
                                    <div class="mt-3">
                                        <div class="text-[10px] font-medium uppercase tracking-wide text-gray-400">Price</div>
