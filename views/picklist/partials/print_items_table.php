...
<table>
    ...
    <tbody>
        <?php if ($printItems === []): ?>
            <?php $emptyColspan = 9 + ($showShortfallColumns ? 2 : 0) + ($showBookColumns ? 2 : 0); ?>
            <tr>
        ...
        <?php foreach ($printItems as $idx => $item): ?>
            ...
            $statusLabel = $availability === 'partial' ? 'Partial' : ($availability === 'none' ? 'Not available' : 'Full');
            ?>
            <tr>
                ...
                <?php if ($showShortfallColumns): ?>
                    ...
                    <td class="<?= $availability === 'none' ? 'status-none' : 'status-partial' ?>"><?= htmlspecialchars($statusLabel) ?></td>
                <?php endif; ?>
                <?php if ($showBookColumns): ?>
