...
(function() {
    ...
    function getActiveContainer() {
        var activePanel = document.querySelector('.js-picklist-tab-panel:not(.hidden)');
        return activePanel || document;
    }
    ...
    function selectedCheckboxes() {
        return getActiveCheckboxes().filter(function(cb) { return cb.checked; });
    }
    ...
    function updateBulkBar() {
        var activeCbs = getActiveCheckboxes();
        ...
        if (selectAll) {
            selectAll.checked = activeCbs.length > 0 && selected === activeCbs.length;
            selectAll.indeterminate = selected > 0 && selected < activeCbs.length;
        }
        updateRowSelectionState();
    }

    window.updatePicklistBulkBar = updateBulkBar;
    ...
    document.addEventListener('click', function(e) {
        var row = e.target.closest('.picklist-select-row');
        if (!row) return;
    ...
    document.addEventListener('change', function(e) {
        if (e.target && e.target.classList.contains('picklist-item-cb')) {
            updateBulkBar();
        }
    });
    ...
    if (bulkPickBtn) {
        bulkPickBtn.addEventListener('click', function() {
            ...
            confirmFn({
                ...
                if (bulkUnpickBtn) bulkUnpickBtn.disabled = true;
    ...
    if (bulkUnpickBtn) {
        bulkUnpickBtn.addEventListener('click', function() {
            ...
            confirmFn({
                ...
                if (!confirmed) return;
                if (bulkPickBtn) bulkPickBtn.disabled = true;
