<?php
function language_nav_query($search, $status_filter, array $extra = [])
{
    $params = array_merge([
        'page' => 'languages',
        'action' => 'list',
            ...
            <div class="min-w-0 max-w-3xl">
                <div class="inline-flex items-center gap-2 rounded-full border border-amber-200/60 bg-white/70 px-3 py-1 text-xs font-semibold text-amber-900/90 shadow-sm backdrop-blur-sm mb-4">
                    <span class="flex h-6 w-6 items-center justify-center rounded-md bg-amber-100 text-amber-700">
                        <i class="fas fa-language text-[11px]" aria-hidden="true"></i>
                    </span>
                    <span>Catalog · Book Languages</span>
                </div>
                <h1 class="text-3xl sm:text-4xl font-bold tracking-tight text-gray-900">
                    Book language <span class="text-amber-800">listing</span>
                </h1>
                <p class="mt-3 text-sm sm:text-base text-gray-600 leading-relaxed max-w-2xl">
                    Manage languages used for books: ISO codes, display names, and active status.
                </p>
            </div>
            <div class="flex shrink-0 lg:pl-4 lg:self-center gap-3 flex-wrap">
                <button type="button" id="open-language-popup-btn"
        ...
        <div class="px-5 py-4 bg-gradient-to-r from-amber-50/50 via-gray-50/90 to-gray-50/90 border-b border-amber-100/80">
            <div class="flex items-center gap-3 min-w-0">
                ...
                <div class="min-w-0">
                    ...
                    <p class="text-xs text-gray-500 mt-0.5 hidden sm:block">Find languages by ISO code, name, or active status.</p>
                </div>
            </div>
        </div>
        <form method="get" id="filterForm" class="p-5">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-5 gap-y-4">
                <div class="sm:col-span-2">
                    ...
                    <input type="text" name="search_text" placeholder="Search by ISO code or language name"
            ...
            <div class="mt-5 flex flex-wrap items-center gap-3">
                ...
                <button type="button" class="inline-flex items-center gap-2 px-5 py-2.5 rounded-lg border border-gray-300 bg-white text-gray-700 text-sm font-medium hover:bg-gray-50 transition"
                    onclick="document.getElementById('filterForm').reset();window.location='?page=languages&action=list';">
                    Reset
                </button>
            </div>
        </form>
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full text-left">
                    ...
                    <tr class="bg-gray-50/95 border-b border-gray-200 text-xs font-semibold uppercase tracking-wider text-gray-600">
                        ...
                        <th scope="col" class="px-5 py-3.5 whitespace-nowrap">ISO</th>
                        <th scope="col" class="px-5 py-3.5 whitespace-nowrap">Language name</th>
                    ...
                    <?php if (!empty($languages_data)): ?>
                        ...
                        <?php foreach ($languages_data as $row): ?>
                            <?php
                            ...
                            <tr class="hover:bg-amber-50/40 transition-colors">
                                ...
                                <td class="px-5 py-4 whitespace-nowrap text-sm font-mono font-semibold text-gray-900 uppercase"><?= htmlspecialchars((string) ($row['iso'] ?? '')) ?></td>
                                <td class="px-5 py-4 text-sm font-semibold text-gray-900"><?= htmlspecialchars((string) ($row['language_name'] ?? '')) ?></td>
                                ...
                                <td class="px-5 py-4 whitespace-nowrap text-sm font-medium">
                                    <div class="menu-wrapper">
                                        ...
                                        <ul class="menu-popup">
                                            ...
                                            <li class="deactivate-btn" data-id="<?php echo (int) $row['id']; ?>" data-name="<?php echo htmlspecialchars((string) ($row['language_name'] ?? ''), ENT_QUOTES, 'UTF-8'); ?>"><i class="fa-solid fa-ban"></i> Deactivate</li>
                                            <li class="permanent-delete-btn text-red-700" data-id="<?php echo (int) $row['id']; ?>" data-name="<?php echo htmlspecialchars((string) ($row['language_name'] ?? ''), ENT_QUOTES, 'UTF-8'); ?>"><i class="fa-solid fa-trash"></i> Delete permanently</li>
                                        </ul>
                                    </div>
                                </td>
                            </tr>
                        ...
                        <tr>
                            <td colspan="6" class="px-6 py-16 text-center text-gray-500">
                                <div class="mx-auto flex max-w-sm flex-col items-center">
                                    ...
                                    <p class="text-base font-medium text-gray-900">No languages match</p>
                                    <p class="mt-1 text-sm text-gray-500">Try changing filters or add a new language.</p>
                                </div>
                            </td>
                        </tr>
                    <?php endif; ?>
            ...
            <div class="flex flex-wrap items-center justify-center gap-3 text-sm text-gray-600">
                <?php if ($total_pages > 1): ?>
                    <a class="inline-flex items-center px-3 py-1.5 rounded-lg border border-gray-300 bg-white hover:bg-gray-50 <?php if ($page <= 1) {
                        ...
                        href="?<?= htmlspecialchars(language_nav_query($search ?? '', $status_filter ?? '', ['page_no' => max(1, $page - $slot_size), 'limit' => $limit])) ?>">
                        &laquo; Prev
                    </a>
                    <?php for ($i = $start; $i <= $end; $i++): ?>
                        <a class="inline-flex items-center px-3 py-1.5 rounded-lg <?= $i === $page ? 'bg-amber-600 text-white font-bold' : 'border border-gray-300 bg-white text-gray-700 hover:bg-gray-50' ?>"
                            href="?<?= htmlspecialchars(language_nav_query($search ?? '', $status_filter ?? '', ['page_no' => $i, 'limit' => $limit])) ?>">
                            <?= (int) $i ?>
                        </a>
                    <?php endfor; ?>
                    <a class="inline-flex items-center px-3 py-1.5 rounded-lg border border-gray-300 bg-white hover:bg-gray-50 <?php if ($page >= $total_pages) {
                        ...
                        href="?<?= htmlspecialchars(language_nav_query($search ?? '', $status_filter ?? '', ['page_no' => min($total_pages, $page + $slot_size), 'limit' => $limit])) ?>">
                        Next &raquo;
                    </a>
                <?php endif; ?>
                <select id="rows-per-page" class="px-2 py-1.5 rounded-lg border border-gray-300 bg-white text-gray-700 focus:outline-none focus:ring-2 focus:ring-amber-500/30 focus:border-amber-500"
                    onchange="location.href='?<?= htmlspecialchars(language_nav_query($search ?? '', $status_filter ?? '', ['page_no' => 1])) ?>&limit=' + encodeURIComponent(this.value);">
...
<div id="popup-wrapper" class="hidden">
    ...
    <div id="modal-slider" class="popup-transition fixed top-0 right-0 h-full flex transform translate-x-full z-50" style="width: 35%; min-width: 400px;">
        <div class="flex-shrink-0 flex items-start pt-5">
            <button id="close-language-popup-btn" type="button" class="bg-white text-gray-800 hover:bg-gray-100 transition flex items-center justify-center shadow-lg" style="width: 61px; height: 61px; border-top-left-radius: 8px; border-bottom-left-radius: 8px;">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12"></path>
                </svg>
            </button>
        </div>
        <div id="language-popup-panel" class="h-full bg-white shadow-2xl" style="width: 100%;">
            <div class="h-full w-full overflow-y-auto">
                <div class="p-6">
                    <h2 class="text-2xl font-bold text-gray-800 mb-6 pb-6 border-b">Add language</h2>
                    <div id="addLanguageMsg" class="text-sm font-bold"></div>
                    <form id="addLanguageForm">
                        <div class="pt-4 space-y-4">
                            <div>
                                <label class="text-sm font-medium text-gray-700">ISO code <span class="text-red-500">*</span></label>
                                <input type="text" class="form-input w-full mt-1 font-mono uppercase" required name="addIso" id="addIso" maxlength="10" placeholder="e.g. en, hi, sa" />
                                <p class="text-xs text-gray-500 mt-1">Standard language code (ISO 639).</p>
                            </div>
                            <div>
                                <label class="text-sm font-medium text-gray-700">Language name <span class="text-red-500">*</span></label>
                                <input type="text" class="form-input w-full mt-1" required name="addLanguageName" id="addLanguageName" placeholder="e.g. English, Hindi, Sanskrit" />
                            </div>
                            <div>
                        ...
                        <div class="flex justify-center items-center gap-4 pt-6 border-t mt-6">
                            <button type="button" id="cancel-language-btn" class="action-btn cancel-btn">Back</button>
...
<!-- Edit Modal -->
<div class="modal fade hidden" id="editLanguageModal" tabindex="-1" aria-hidden="true">
  <div class="modal-dialog">
    ...
    <div id="modal-slider-edit" class="popup-transition fixed top-0 right-0 h-full flex transform translate-x-full z-50" style="width: 35%; min-width: 400px;">
        <div class="flex-shrink-0 flex items-start pt-5">
            <button id="close-language-popup-btn-edit" type="button" class="bg-white text-gray-800 hover:bg-gray-100 transition flex items-center justify-center shadow-lg" style="width: 61px; height: 61px; border-top-left-radius: 8px; border-bottom-left-radius: 8px;">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12"></path>
                </svg>
            </button>
        ...
        <div class="h-full bg-white shadow-2xl" style="width: 100%;">
            <div class="h-full w-full overflow-y-auto">
                <div class="p-8">
                    <h2 class="text-2xl font-bold text-gray-800 mb-6 pb-6 border-b">Edit language</h2>
                    <div id="editLanguageMsg"></div>
                    <form id="editLanguageForm">
                        <input type="hidden" id="editId" name="id" value="">
                        <div class="pt-4 space-y-4">
                            <div>
                                <label class="text-sm font-medium text-gray-700">ISO code <span class="text-red-500">*</span></label>
                                <input type="text" class="form-input w-full mt-1 font-mono uppercase" required name="editIso" id="editIso" maxlength="10" />
                            </div>
                            <div>
                                <label class="text-sm font-medium text-gray-700">Language name <span class="text-red-500">*</span></label>
                                <input type="text" class="form-input w-full mt-1" required name="editLanguageName" id="editLanguageName" />
                            </div>
                            <div>
                                <label class="text-sm font-medium text-gray-700">Status <span class="text-red-500">*</span></label>
                        ...
                        <div class="flex justify-center items-center gap-4 pt-6 border-t mt-6">
                            <button type="button" id="cancel-language-btn-edit" class="action-btn cancel-btn">Back</button>
                            <button type="submit" class="action-btn save-btn">Save</button>
                        </div>
                    </form>
                </div>
...
<script>
    ...
    const openLanguagePopupBtn = document.getElementById('open-language-popup-btn');
    const popupWrapper = document.getElementById('popup-wrapper');
    const modalSlider = document.getElementById('modal-slider');
    const cancelLanguageBtn = document.getElementById('cancel-language-btn');
    const closeLanguagePopupBtn = document.getElementById('close-language-popup-btn');

    function openLanguagePopup() {
        document.getElementById('addLanguageForm').reset();
        document.getElementById('addLanguageMsg').innerHTML = '';
    ...
    openLanguagePopupBtn.addEventListener('click', openLanguagePopup);
    cancelLanguageBtn.addEventListener('click', closeLanguagePopup);
    closeLanguagePopupBtn.addEventListener('click', closeLanguagePopup);

    document.getElementById('addLanguageForm').onsubmit = function(e) {
        e.preventDefault();
        const form = new FormData(this);
        const params = new URLSearchParams(form).toString();
        fetch('?page=languages&action=addRecord', {
            method: 'POST',
        ...
        .then(r => r.json())
        .then(data => {
            const msgBox = document.getElementById('addLanguageMsg');
            msgBox.innerHTML = '';
            if (data.success) {
    ...
    function showLanguageActionResult(data) {
    ...
    document.addEventListener('DOMContentLoaded', () => {
        document.querySelectorAll('.deactivate-btn').forEach(btn => {
            btn.addEventListener('click', async (e) => {
                ...
                fetch('?page=languages&action=deleteRecord', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ id: id })
                })
                .then(res => res.json())
                .then(showLanguageActionResult);
            });
        });
        ...
        document.querySelectorAll('.permanent-delete-btn').forEach(btn => {
            btn.addEventListener('click', async (e) => {
                ...
                fetch('?page=languages&action=permanentDelete', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ id: id })
                })
    ...
    const popupWrapperEdit = document.getElementById('editLanguageModal');
    const modalSliderEdit = document.getElementById('modal-slider-edit');
    const cancelLanguageBtnEdit = document.getElementById('cancel-language-btn-edit');
    const closeLanguagePopupBtnEdit = document.getElementById('close-language-popup-btn-edit');

    function openEditModal(id) {
        closeAllMenus();
        fetch('?page=languages&action=getDetails&id=' + id)
        .then(res => res.json())
        .then(data => {
            ...
            document.getElementById('editIso').value = data.iso || '';
            document.getElementById('editLanguageName').value = data.language_name || '';
            document.getElementById('editStatus').value = data.active ? '1' : '0';
    ...
    function closeLanguagePopupEdit() {
        modalSliderEdit.classList.add('translate-x-full');
    }

    closeLanguagePopupBtnEdit.addEventListener('click', closeLanguagePopupEdit);
    cancelLanguageBtnEdit.addEventListener('click', closeLanguagePopupEdit);
    ...
    document.getElementById('editLanguageForm').onsubmit = function(e) {
        e.preventDefault();
        const form = new FormData(this);
        const params = new URLSearchParams(form).toString();
        fetch('?page=languages&action=addRecord', {
        ...
        .then(r => r.json())
        .then(data => {
            const msgBox = document.getElementById('editLanguageMsg');
            msgBox.innerHTML = '';
            if (data.success) {
                msgBox.innerHTML = '<div style="color: green; padding: 10px; background: #e0ffe0; border: 1px solid #0a0;">✅ ' + data.message + '</div>';
                setTimeout(() => { window.location.href = '?page=languages&action=list'; }, 1000);
            } else {
                msgBox.innerHTML = '<div style="color: red; padding: 10px; background: #ffe0e0; border: 1px solid #a00;">❌ ' + data.message + '</div>';
            }
        });
