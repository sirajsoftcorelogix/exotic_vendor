...
<div class="max-w-7xl mx-auto px-4 sm:px-6 py-8">
        ...
        <div class="relative px-5 py-7 sm:px-8 sm:py-9 flex flex-col lg:flex-row lg:items-center lg:justify-between gap-8">
            <div class="min-w-0 max-w-3xl">
                ...
                <h1 class="text-3xl sm:text-4xl font-bold tracking-tight text-gray-900">
                    In transit <span class="text-amber-800">inventory</span>
                </h1>
            ...
            <div class="flex shrink-0 lg:pl-4 lg:self-center gap-3 flex-wrap">
                <a href="?page=products&action=stock_transfer"
                    ...
                    <i class="fas fa-history text-xs opacity-95" aria-hidden="true"></i>
                    Transfer history
                </a>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4 mb-6">
        <div class="rounded-2xl border border-amber-100 bg-white p-5 shadow-sm ring-1 ring-gray-900/[0.03]">
            <div class="text-[11px] font-bold uppercase tracking-wider text-amber-800/80">Units in transit</div>
        ...
        <div class="mb-6 flex flex-wrap items-center gap-2">
            ...
            <?php foreach ($summary['by_destination'] as $dest): ?>
                ...
                <a href="<?php echo htmlspecialchars($destUrl); ?>"
                    class="inline-flex items-center gap-1.5 rounded-full border border-amber-200 bg-amber-50/80 px-3 py-1 text-xs font-medium text-amber-900 hover:bg-amber-100 transition">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 px-4 sm:px-5 border-b border-gray-100">
                ...
                ?>
                    <a href="<?php echo htmlspecialchars($viewTabBase($tabKey)); ?>"
                        class="<?php echo $isActive
                            ? 'border-amber-500 text-amber-800 font-bold border-b-2'
            ...
            <form method="GET" class="flex items-center gap-2 py-3 sm:py-0">
                ...
                <select id="itSort" name="sort" onchange="this.form.submit()"
                    class="text-sm border border-gray-300 rounded-lg px-3 py-2 bg-white focus:ring-2 focus:ring-amber-500/30 focus:border-amber-500">
                    <?php if ($view === 'item'): ?>
    ...
    <style>
        ...
        #it-filters[open] > summary { border-bottom: 1px solid rgba(251, 243, 219, 0.85); }
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-x-5 gap-y-4">
                ...
                <?php if ($userWarehouseId > 0): ?>
                    <div>
                        ...
                        <select name="direction" class="w-full px-3 py-2.5 border border-gray-300 rounded-lg text-sm text-gray-900 bg-white shadow-sm focus:ring-2 focus:ring-amber-500/30 focus:border-amber-500 transition">
                ...
                <div>
                    ...
                    <select name="receipt_state" class="w-full px-3 py-2.5 border border-gray-300 rounded-lg text-sm text-gray-900 bg-white shadow-sm focus:ring-2 focus:ring-amber-500/30 focus:border-amber-500 transition">
                    ...
                    <tbody class="divide-y divide-gray-100">
                            ...
                            <tr>
                                <td colspan="8" class="px-5 py-16 text-center">
                                    <div class="mx-auto flex max-w-sm flex-col items-center">
                                        <span class="inline-flex h-14 w-14 items-center justify-center rounded-full bg-amber-50 text-amber-400 text-xl mb-4">
                                        ...
                                        <a href="?page=products&action=stock_transfer" class="mt-5 inline-flex items-center gap-2 text-sm font-semibold text-amber-700 hover:text-amber-800">
                        ...
                        <?php else: ?>
                            <?php foreach ($records as $row): ?>
                                ...
                                ?>
                                <tr class="hover:bg-amber-50/40 transition-colors it-item-row" data-item-key="<?php echo htmlspecialchars($itemKey, ENT_QUOTES, 'UTF-8'); ?>">
                                    ...
                                    <td class="px-4 py-4 align-top">
                                        <span class="text-lg font-bold tabular-nums text-amber-800"><?php echo number_format($pending); ?></span>
                                    </td>
                                    ...
                                    <td class="w-0 px-2 py-4 align-middle text-center whitespace-nowrap">
                                        <?php if (!empty($transfers[0]['transfer_id'])): ?>
                                            <a href="?page=products&action=stock_transfer_items&transfer_id=<?php echo urlencode((string) $transfers[0]['transfer_id']); ?>"
                                                ...
                                                class="inline-flex h-7 w-7 items-center justify-center rounded border border-gray-200 bg-white text-amber-700 hover:bg-gray-50"
                                                    ...
                                                    <tbody class="divide-y divide-gray-100">
                                                        <?php foreach ($transfers as $tr): ?>
                                                            ...
                                                            <tr>
                                                                ...
                                                                <td class="px-4 py-2.5 text-right tabular-nums font-semibold text-amber-800"><?php echo number_format((int) ($tr['pending_qty'] ?? 0)); ?></td>
                            ...
                            <?php foreach ($records as $transfer): ?>
                                ...
                                <tr class="hover:bg-amber-50/40 transition-colors">
                                    ...
                                    <td class="px-5 py-4 align-top">
                                        ...
                                        <?php else: ?>
                                            <span class="inline-flex items-center rounded-full bg-amber-50 px-2.5 py-0.5 text-xs font-medium text-amber-800 ring-1 ring-inset ring-amber-600/20">Not received</span>
                                        <?php endif; ?>
                                    </td>
                                    <td class="px-5 py-4 align-top text-sm text-gray-700">
                                        <div class="flex flex-col gap-1">
                                            ...
                                            <span class="inline-flex items-start gap-1.5">
                                                <i class="fas fa-arrow-down text-amber-700 mt-0.5 text-xs" aria-hidden="true"></i>
                                                <?php echo htmlspecialchars((string) ($transfer['dest_name'] ?? '—')); ?>
                                            </span>
                                        </div>
                                    </td>
                                    <td class="px-5 py-4 align-top">
                                        <div class="text-lg font-bold tabular-nums text-amber-800"><?php echo number_format($pending); ?></div>
