...
$saved_shippingfee = trim((string) ($data['form2']['shippingfee'] ?? ''));
$book_shipping_fee_initial = $saved_shippingfee !== ''
    ? round((float) $saved_shippingfee, 2)
    : (function_exists('book_shipping_fee_inr')
...
<div class="w-full max-w-[1200px] mx-auto p-2 md:p-5 font-['Segoe_UI',Tahoma,Geneva,Verdana,sans-serif] text-[#333]">
    ...
    <?php
    $desktopformFlash = $_SESSION['desktopform_flash'] ?? null;
    unset($_SESSION['desktopform_flash']);
    if (is_array($desktopformFlash) && !empty($desktopformFlash['text'])):
    ?>
    <div class="mb-3 rounded-md border border-red-200 bg-red-50 px-4 py-3 text-sm font-semibold text-red-700" role="alert">
        <?php echo htmlspecialchars((string) $desktopformFlash['text'], ENT_QUOTES, 'UTF-8'); ?>
    </div>
    <?php endif; ?>
        ...
        <div class="mb-8 border-b-2 border-gray-100 pb-6 w-full">
            ...
            <div class="bg-gray-50 p-5 rounded border border-gray-200 w-full" id="inbound-section-item-details">
                ...
                <div id="main-item-details-grid" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-6 gap-6 w-full">
                    ...
                    <div class="w-full min-w-0">
                        <label class="block text-xs font-bold text-[#555] mb-1">Width/Breadth:</label>
                        <div class="relative w-full">
                    ...
                    <div class="lg:col-span-2 w-full min-w-0">
                        <label class="block text-xs font-bold text-[#555] mb-1">Dimensions:</label>
                        <div class="relative w-full">
                            <input type="text" id="dimensions" class="w-full h-10 border border-[#ccc] rounded-[3px] px-3 text-[13px] text-[#333] focus:outline-none focus:border-[#d97824]" value="<?= htmlspecialchars($data['form2']['dimensions'] ?? '') ?>" name="dimensions" placeholder="Dimensions">
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-5 gap-4">
                            ...
                            <div class="md:col-span-5">
                                ...
                                <textarea
                                    ...
                                    autocomplete="off"
                                    rows="2"
                                    class="w-full min-h-[2.5rem] border border-[#ccc] rounded-[3px] px-3 py-2 text-[13px] leading-relaxed text-gray-600 bg-gray-100 cursor-not-allowed focus:outline-none resize-none"
                                    title="Auto-generated from selected language roles"><?php echo htmlspecialchars($saved_combined_language, ENT_QUOTES, 'UTF-8'); ?></textarea>
                            </div>
                            <div>
                                <label class="block text-xs font-bold text-[#555] mb-1">Pages</label>
                                <input type="text" name="pages" value="<?php echo htmlspecialchars($data['form2']['pages'] ?? ''); ?>" placeholder="e.g. 10 pages color" class="w-full h-10 border border-[#ccc] rounded-[3px] px-3 text-[13px] text-[#333] focus:outline-none focus:border-[#d97824] bg-white">
                            </div>
                            <div>
                ...
                <div class="variation-card calculation-card mb-8 border-b-2 border-gray-100 pb-6 w-full last:border-0 last:pb-0 last:mb-0 relative group" data-id="<?= $var['id'] ?>">
                    ...
                    <div class="bg-white p-5 rounded border border-gray-200 w-full shadow-sm">
                        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-6 gap-6 w-full">
                            ...
                            <div class="w-full min-w-0"><label class="block text-xs font-bold text-[#555] mb-1">Store Location:</label><input type="text" class="w-full h-10 border border-[#ccc] rounded-[3px] px-3 text-[13px] text-[#333] focus:outline-none focus:border-[#d97824]" value="<?= htmlspecialchars((isset($var['store_location']) && trim((string)$var['store_location']) !== '') ? $var['store_location'] : '4th Floor') ?>" name="variations[<?= $var['id'] ?>][store_location]"></div>
    ...
    <div class="variation-card calculation-card mb-8 border-b-2 border-gray-100 pb-6 w-full pt-6 border-t-2 mt-6 relative group">
        ...
        <div class="bg-white p-5 rounded border border-gray-200 w-full shadow-sm">
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-6 gap-6 w-full">
                ...
                <div class="w-full min-w-0"><label class="block text-xs font-bold text-[#555] mb-1">Store Location:</label><input type="text" class="w-full h-10 border border-[#ccc] rounded-[3px] px-3 text-[13px] text-[#333] focus:outline-none focus:border-[#d97824]" name="variations[INDEX][store_location]" value="4th Floor"></div>
                    ...
                    <div id="invoice_preview_container" class="flex-1 <?php echo $hasImage ? '' : 'hidden'; ?>">
                        ...
                        <div class="border border-[#ccc] rounded-[5px] p-3 flex gap-4 items-center bg-white h-[100px]">
                            ...
                            <div class="flex flex-col gap-2 justify-center items-start">
                                ...
                                <button type="button" id="upload_invoice_btn" onclick="uploadInvoiceNow()" class="hidden inline-flex items-center gap-1.5 px-3 py-1.5 bg-[#d97824] text-white text-[11px] font-bold rounded shadow hover:bg-[#c66a1d] transition mt-1">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 items-start mb-[15px]">
                    <div class="flex-1">
                        ...
                        <select class="w-full h-[32px] border border-[#ccc] rounded-[3px] px-[10px] text-[13px] text-[#333] focus:outline-none focus:border-[#999]" name="permanently_available">
                            <?php $perm = ((int) ($data['form2']['permanently_available'] ?? 0) === 1) ? 1 : 0; ?>
...
window.bookShippingFeeManual = <?php echo json_encode($saved_shippingfee !== ''); ?>;

window.updateBookShippingFeeFromWeight = function (force) {
    const input = document.getElementById('book_shippingfee');
    if (!input) {
        return;
    }
    if (window.bookShippingFeeManual && !force) {
        return;
    }
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const shippingFeeInput = document.getElementById('book_shippingfee');
    if (shippingFeeInput) {
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const sSubCatContainer = document.getElementById('search_sub_category_container');
    const sSubSubCatContainer = document.getElementById('search_sub_sub_category_container');
    // 3. Reuse Search Group Tom Select (first script block already inits #search_group_select)
    const config = { create: false, sortField: { field: "text", direction: "asc" }, controlInput: null };
    let sGroupTs = null;
    if (sGroupSelectEl) {
        if (sGroupSelectEl.tomselect) {
            sGroupTs = sGroupSelectEl.tomselect;
        } else if (typeof window.safeTomSelect === 'function') {
            sGroupTs = window.safeTomSelect(sGroupSelectEl, config);
        }
    }
