...
if (!empty($form2['edited_by']) && isset($inboundingModel) && method_exists($inboundingModel, 'parseInboundAuthorIds')) {
    foreach ($inboundingModel->parseInboundAuthorIds($form2['edited_by']) as $editorId) {
        ...
        <form action="<?php echo $formAction; ?>" method="POST" enctype="multipart/form-data" id="mainForm" class="p-4 space-y-6">
            ...
            <div id="book-meta-fields" class="hidden bg-gray-50 border border-gray-300 rounded p-4">
                ...
                <div class="grid grid-cols-1 md:grid-cols-5 gap-4">
                    ...
                    <div>
                        <label class="block text-gray-800 font-bold text-xs mb-1">Edited By</label>
                        ...
                        <select id="edited_by_select" multiple class="w-full border border-gray-400 rounded px-2 py-2 text-sm focus:border-black outline-none bg-white">
                    ...
                    <div class="variation-card border border-gray-300 rounded-lg p-3 bg-gray-50/50" data-index="<?php echo $index; ?>">
                        ...
                        <div class="flex flex-col md:flex-row gap-6">
                            ...
                            <div class="flex-1 grid grid-cols-2 md:grid-cols-6 gap-x-4 gap-y-4 items-start">
                                ...
                                <div>
                                    <label class="block text-xs font-bold text-black mb-1">Store Location:</label>
                                    <input type="text" name="variations[<?php echo $index; ?>][store_location]" value="<?php echo (isset($var['store_location']) && trim((string)$var['store_location']) !== '') ? $var['store_location'] : '4th Floor'; ?>" class="w-full border border-gray-400 rounded px-2 py-1.5 text-sm focus:border-black outline-none">
                                </div>
                                <div>
...
<?php require __DIR__ . '/partials/isbn_lookup_modal.php'; ?>
<?php require __DIR__ . '/partials/isbn_lookup_script.php'; ?>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const editedBySelectEl = document.getElementById('edited_by_select');
        ...
        if (editedBySelectEl) {
            ...
            editedByTomSelect = new TomSelect(editedBySelectEl, {
                plugins: ['remove_button'],
                valueField: 'id',
                labelField: 'name',
                searchField: ['name'],
                ...
                load: function (query, callback) {
                    ...
                    fetch(searchAuthorsUrl + encodeURIComponent(query))
                        .then(function (response) {
                            if (!response.ok) {
                                throw new Error('Edited-by request failed: ' + response.status);
                            }
                            return response.json();
                        })
                        .then(function (json) { callback(mapAuthorOptions(json)); })
                        .catch(function (err) {
                            console.error('Edited-by load error:', err);
                            callback();
                        });
                }
            });
