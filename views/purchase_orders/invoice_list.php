<div class="mx-auto space-y-6 mr-4">
    <h2 class="text-2xl font-bold my-4">Vendor Invoices</h2>
    <!--Advance Search-->
        ...
        <div id="accordion-content" class="accordion-content hidden">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 2xl:grid-cols-6 gap-6 items-end">
                ...
                <div class="col-span-1 sm:col-span-2 md:col-span-3 lg:col-span-2 flex items-end gap-2">    
                    <div class="w-1/2">
                        <label for="invoice-date_from" class="block text-sm font-medium text-gray-600 mb-1">Invoice Date</label>
                        <input type="date" name="invoice_date_from" id="invoice-date_from" placeholder="Invoice Date" class="w-full px-2 py-1 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-1 focus:ring-amber-500 focus:border-amber-500">
                    </div>
                    <span class="text-gray-500 pb-2">→</span>
                    <div class="w-1/2">
                        <label for="invoice-date-to" class="block text-sm font-medium text-gray-600 mb-1">To</label>
                        <input type="date" name="invoice_date_to" id="invoice-date-to" placeholder="To Date" class="w-full px-2 py-1 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-1 focus:ring-amber-500 focus:border-amber-500">
                    </div>
                </div>
                 <!-- Min/Max Amount -->
            ...
            <!-- clear filter -->
             <script>
                function clearFilters() {
                    ...
                    const page = 'page=orders&action=invoice_list'; // Retain only page and action parameters
        ...
        <div class="p-6 ">
            <div class="table-container">
                ...
                <tr>
                    ...
                    <td class="px-6 py-4 whitespace-nowrap" title="<?= htmlspecialchars($invoice['vendor_phone'] ?? '') ?>  <?= htmlspecialchars($invoice['vendor_city'] ?? '') ?>"><?= htmlspecialchars($invoice['vendor_name'] ?? '') ?></td>
            ...
            <div class="flex items-center justify-center">
                <div id="pagination-controls" class="flex items-center gap-4 text-sm text-gray-600">
                    ...
                    <!-- Prev Button -->
                        ...
                        href="?page=orders&action=invoice_list&page_no=<?= $page-$slot_size ?>&limit=<?= $limit ?><?= $query_string ?>">
                            &laquo; Prev
                        </a>
                        <!-- Page Slots -->
                        <?php for ($i = $start; $i <= $end; $i++): ?>
                            <a class="page-link px-2 py-1 rounded <?= $i == $page ? 'bg-black text-white font-bold' : 'bg-gray-100 text-gray-700' ?>"
                            href="?page=orders&action=invoice_list&page_no=<?= $i ?>&limit=<?= $limit ?><?= $query_string ?>">
                                <?= $i ?>
                            </a>
                        <?php endfor; ?>
                        <!-- Next Button -->
                    ...
                    <select id="rows-per-page" class="pagination-select bg-transparent border-b border-gray-400 focus:outline-none focus:border-gray-800 text-gray-600"
                            onchange="location.href='?page=orders&action=invoice_list&page_no=1&limit=' + this.value;">
