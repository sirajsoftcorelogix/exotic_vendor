...
$items = $data['items'] ?? [];
?>
<div class="export-doc-page declaration-audio">
    <div class="doc-header text-center border-b-2 border-black pb-3 mb-4">
        <h1 class="text-xl font-bold uppercase tracking-wider text-black"><?= htmlspecialchars($data['document_title'] ?? 'AUDIO / FILM DECLARATION') ?></h1>
        <p class="text-xs text-gray-700">Copyright & Content Declaration for Recorded Media Export</p>
    </div>
    ...
    <table class="w-full text-left text-xs border-collapse border border-black mb-4">
        <thead>
            <tr class="bg-gray-100 font-bold border-b border-black">
                ...
                <th class="border border-black p-1.5">Album / Title</th>
                <th class="border border-black p-1.5 w-36">Media Type</th>
        ...
        <tbody>
            <?php foreach ($items as $idx => $it): ?>
                <tr>
                    ...
                    <td class="border border-black p-1.5"><?= htmlspecialchars($data['media_type'] ?? 'Compact Disc (Audio CD) / DVD') ?></td>
                    <td class="border border-black p-1.5 text-center"><?= htmlspecialchars($it['hsn_code'] ?? '85234910') ?></td>
    ...
    <div class="border border-black p-4 text-xs space-y-3 mb-4 leading-relaxed text-justify">
        ...
        <p>
            <?= htmlspecialchars($data['copyright_statement'] ?? 'We certify that we hold valid distribution rights/licenses for the audio/film content recorded on these media discs and they do not violate copyright laws or contain prohibited content under Indian law.') ?>
        </p>
    </div>
