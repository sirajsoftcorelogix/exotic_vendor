...
$items = $data['items'] ?? [];
?>
<div class="export-doc-page declaration-lacey">
    <div class="doc-header text-center border-b-2 border-black pb-3 mb-4">
        <h1 class="text-xl font-bold uppercase tracking-wider text-black"><?= htmlspecialchars($data['document_title'] ?? 'LACEY ACT DECLARATION (US CUSTOMS)') ?></h1>
        <p class="text-xs text-gray-700">Plant & Plant Product Declaration Form for US Customs Entry</p>
    </div>

    <div class="border border-black p-3 text-xs space-y-1 mb-4 bg-gray-50">
        <div class="flex justify-between">
            <div><strong>Shipper / Exporter:</strong> <?= htmlspecialchars($common['exporter_name'] ?? '') ?></div>
            <div><strong>Invoice No:</strong> <?= htmlspecialchars($common['invoice_number'] ?? 'N/A') ?></div>
        </div>
        <div class="flex justify-between">
            <div><strong>Consignee:</strong> <?= htmlspecialchars($common['consignee_name'] ?? '') ?> (USA)</div>
    ...
    <table class="w-full text-left text-xs border-collapse border border-black mb-4">
        <thead>
            <tr class="bg-gray-100 font-bold border-b border-black">
                ...
                <th class="border border-black p-1.5">Description of Wood Item</th>
                <th class="border border-black p-1.5 w-40">Scientific Genus & Species</th>
                <th class="border border-black p-1.5 w-28 text-center">Country Harvested</th>
        ...
        <tbody>
            <?php foreach ($items as $idx => $it): ?>
                <tr>
                    ...
                    <td class="border border-black p-1.5 font-italic"><?= htmlspecialchars($data['genus_species'] ?? 'Tectona grandis / Dalbergia sissoo') ?></td>
                    <td class="border border-black p-1.5 text-center"><?= htmlspecialchars($data['country_harvested'] ?? 'India') ?></td>
    ...
    <div class="border border-black p-4 text-xs space-y-3 mb-4 leading-relaxed text-justify">
        ...
        <p>
            <?= htmlspecialchars($data['lacey_statement'] ?? 'I certify under penalty of perjury that the information provided in this Lacey Act Declaration is true and correct to the best of my knowledge.') ?>
        </p>
    </div>
