      ...
      <nav id="main-nav" class="flex-1 overflow-y-auto p-4">
            ...
            <?php
                  ...
                  if (!function_exists('renderMenu')) {
                        ...
                        {
                              $active = ($currentPage === 'dashboard' && $currentAction === 'dashboard') ? 'active' : '';
                              echo '<ul class="mt-1">';
                              echo '<li>';
                              echo '<a href="index.php?page=dashboard&action=dashboard" class="nav-link text-gray-800 ' . $active . '">';
                              ...
                              foreach ($menu as $item) {
                                    ...
                                    foreach ($children as $child) {
                                          ...
                                          $active = ($currentPage === ($child['slug'] ?? '') && $currentAction === ($child['action'] ?? ''))
                                                ...
                                                : '';
                                          echo '<li>';
                                          echo '<a href="index.php?page=' . rawurlencode((string) ($child['slug'] ?? '')) . '&action=' . rawurlencode((string) ($child['action'] ?? '')) . '" class="nav-link text-gray-800 ' . $active . '">';
                  ...
                  $menu = $navigationMenuModel->buildMenuTree($userRoleId);
                  renderMenu($menu, $page, $action);
            ?>
      </nav>
</aside>
<script>
