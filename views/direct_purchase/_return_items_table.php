    ...
    <table class="min-w-full text-xs">
        <thead>
            <tr class="text-gray-500 uppercase tracking-wide">
                ...
                <th class="py-2 pr-4 text-left font-semibold">Variant</th>
        ...
        <tbody class="divide-y divide-gray-100/80">
            <?php foreach ($items as $ln): ?>
                <?php
                $color = trim((string) ($ln['color'] ?? ''));
                $size = trim((string) ($ln['size'] ?? ''));
                $variant = $color !== '' && $size !== '' ? $color . ' / ' . $size : ($color !== '' ? $color : ($size !== '' ? $size : '—'));
                ?>
                <tr>
                    ...
                    <td class="py-2 pr-4 text-gray-700"><?= htmlspecialchars($variant) ?></td>
