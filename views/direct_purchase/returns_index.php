...
$filtersPanelOpen =
    ...
    || trim((string) ($filters['return_date_from'] ?? '')) !== ''
    || trim((string) ($filters['return_date_to'] ?? '')) !== ''
...
<div class="max-w-7xl mx-auto px-4 sm:px-6 py-8">
        ...
        <div class="relative px-5 py-7 sm:px-8 sm:py-9 flex flex-col lg:flex-row lg:items-center lg:justify-between gap-8">
            <div class="min-w-0 max-w-3xl">
                ...
                <p class="mt-3 text-sm sm:text-base text-gray-600 leading-relaxed max-w-2xl">
                    All stock returns recorded against direct purchase invoices.
                </p>
            </div>
            <div class="flex shrink-0 lg:pl-4 lg:self-center">
                <a href="?page=direct_purchase&action=list"
                    class="inline-flex items-center justify-center gap-2 px-6 py-3.5 rounded-xl border border-gray-300 bg-white text-gray-700 text-sm font-semibold hover:bg-gray-50 transition whitespace-nowrap w-full sm:w-auto">
    ...
    <style>
        #dp-return-filters > summary { list-style: none; }
        #dp-return-filters > summary::-webkit-details-marker { display: none; }
        #dp-return-filters[open] > summary { border-bottom: 1px solid rgba(251, 243, 219, 0.85); }
        #dp-return-filters:not([open]) .dpf-label-open { display: none; }
        #dp-return-filters[open] .dpf-label-closed { display: none; }
        #dp-return-filters[open] .dpf-chevron { transform: rotate(180deg); }
    </style>

    <details id="dp-return-filters" class="bg-white rounded-2xl border border-gray-200/80 shadow-sm overflow-hidden mb-6 ring-1 ring-gray-900/[0.03]" <?= $filtersPanelOpen ? 'open' : '' ?>>
        <summary class="px-5 py-4 bg-gradient-to-r from-amber-50/50 via-gray-50/90 to-gray-50/90 flex items-center justify-between gap-4 cursor-pointer">
            <div class="flex items-center gap-3 min-w-0">
                ...
                <div class="min-w-0">
                    ...
                    <p class="text-xs text-gray-500 mt-0.5 hidden sm:block">Keyword, vendor, return dates, and page size.</p>
                </div>
            </div>
        ...
        <form method="GET" action="" class="p-5">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-x-5 gap-y-4">
                ...
                <div>
                    <label class="block text-xs font-semibold text-gray-600 mb-1">Return from</label>
                    <input type="date" name="return_date_from" value="<?= htmlspecialchars($filters['return_date_from'] ?? '') ?>"
                ...
                <div>
                    <label class="block text-xs font-semibold text-gray-600 mb-1">Return to</label>
                    <input type="date" name="return_date_to" value="<?= htmlspecialchars($filters['return_date_to'] ?? '') ?>"
                        max="<?= htmlspecialchars($dpFilterDateMax) ?>"
                        class="w-full px-3 py-2.5 border border-gray-300 rounded-lg text-sm text-gray-900 shadow-sm focus:ring-2 focus:ring-amber-500/30 focus:border-amber-500 transition">
                </div>
            </div>
            <div class="mt-5 flex flex-wrap items-center gap-3">
                ...
                <a href="?page=direct_purchase&action=returns" class="inline-flex items-center gap-2 px-5 py-2.5 rounded-lg border border-gray-300 bg-white text-gray-700 text-sm font-medium hover:bg-gray-50 transition">
                    Reset
                </a>
            </div>
        </form>
                        ...
                        <tr>
                            <td colspan="6" class="px-5 py-16 text-center">
                                <div class="mx-auto flex max-w-sm flex-col items-center">
                                    ...
                                    <p class="text-base font-medium text-gray-900">No returns match</p>
                                    <p class="mt-1 text-sm text-gray-500">Try adjusting filters or record a return from a direct purchase.</p>
                                    <a href="?page=direct_purchase&action=list" class="mt-5 inline-flex items-center gap-2 text-sm font-semibold text-amber-700 hover:text-amber-800">
                            ...
                            <tr class="hover:bg-amber-50/40 transition-colors">
                                ...
                                <td class="px-5 py-4 align-top">
                                    <a href="?page=direct_purchase&amp;action=return_list&amp;dp_id=<?= $dpId ?>"
                                        ...
                                        <?= htmlspecialchars((string) ($r['invoice_number'] ?? '')) ?>
                                    </a>
                                </td>
                                ...
                                <td class="px-5 py-4 align-top text-sm text-right font-medium text-gray-900 tabular-nums">
                                    ...
                                    <span class="inline-flex items-baseline justify-end gap-1.5 flex-wrap">
                                        ...
                                        <span><?= number_format((float) ($r['grand_total'] ?? 0), $curDec) ?></span>
                                ...
                                <td class="w-0 px-2 py-4 align-middle text-center whitespace-nowrap">
                                    <div class="flex items-center justify-center gap-1.5">
                                        <a href="?page=direct_purchase&amp;action=return_list&amp;dp_id=<?= $dpId ?>"
                                            ...
                                            aria-label="View purchase returns">
