...
<div class="mx-auto max-w-[1300px] space-y-6 px-3 py-6 md:px-6 font-sans text-slate-800">
    ...
    <div class="flex flex-col gap-4 rounded-2xl bg-white p-5 shadow-[0px_10px_15px_-3px_#0000001A] md:flex-row md:items-center md:justify-between">
        ...
        <div class="flex flex-wrap items-center gap-3">
            ...
            <?php if ($isExport): ?>
                <a href="<?= htmlspecialchars(base_url('index.php?page=export_documents&query=' . rawurlencode($invoiceNumber !== '' ? $invoiceNumber : $invoiceId)), ENT_QUOTES, 'UTF-8') ?>" target="_blank" class="inline-flex items-center gap-2 rounded-lg bg-sky-600 px-4 py-2.5 text-sm font-semibold text-white hover:bg-sky-700 shadow-md transition">
                    <i class="fas fa-globe-americas"></i> Generate Export Documents
                </a>
            <?php endif; ?>
            <a href="<?= htmlspecialchars(base_url('?page=posinvoice&action=print-preview&invoice_id=' . (int)$invoiceId . '&autoprint=1'), ENT_QUOTES, 'UTF-8') ?>" target="_blank" rel="noopener noreferrer" class="inline-flex items-center gap-2 rounded-lg border border-orange-200 bg-orange-50 px-4 py-2.5 text-sm font-semibold text-orange-800 hover:bg-orange-100 shadow-sm transition">
        ...
        <?php if ($isExport && !empty($intl)): ?>
            <div class="p-6 border-b border-slate-200 bg-sky-50/40">
                <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-3">
                    <h3 class="text-xs font-bold uppercase tracking-wider text-sky-900 flex items-center gap-2">
                        <i class="fas fa-plane-departure text-sky-600"></i> Export &amp; Shipment Details
                    </h3>
                    <a href="<?= htmlspecialchars(base_url('index.php?page=export_documents&query=' . rawurlencode($invoiceNumber !== '' ? $invoiceNumber : $invoiceId)), ENT_QUOTES, 'UTF-8') ?>" target="_blank" class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-sky-600 text-white text-xs font-semibold hover:bg-sky-700 shadow-sm transition">
                        <i class="fas fa-file-export"></i> Generate Export Documents
                    </a>
                </div>
